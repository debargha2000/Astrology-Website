...
// 6c. ASTRO CONTENT
// ==========================================
app.get('/api/astro-content', authenticateToken, async (_req: Request, res: Response) => {
  try {
...
app.post('/api/astro-content', authenticateToken, async (req: Request, res: Response) => {
  try {
...
app.put('/api/astro-content/:id', authenticateToken, async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const { type, key, title, interpretation, updatedBy } = req.body as {
      type?: string;
...
app.delete('/api/astro-content/:id', authenticateToken, async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
...
app.post('/api/astro-content/bulk-seed', authenticateToken, async (_req: Request, res: Response) => {
  try {
    const { buildAstroDefaults } = await import('./astroDefaults');
